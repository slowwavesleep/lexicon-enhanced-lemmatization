...
from lexenlem.models.lemma.data import CoNNLDataset, BatchItem
from lexenlem.models.lemma.trainer import Trainer
from lexenlem.models.lemma import scorer
...
def train(args):
    ...
    dev_dataset = CoNNLDataset(
        args["eval_file"], args, lemmatizer=lemmatizer, vocab=vocab, evaluation=True, device=device
    )
    ...
    if args["lemmatizer"] == "lexicon":
        trainer.lexicon = train_dataset.lemmatizer
    print("[Training dictionary-based lemmatizer...]")
    trainer.train_dict(train_dataset.conll.get(["word", "upos", "lemma"]))
    print("Evaluating on dev set...")
    dev_preds = trainer.predict_dict(dev_dataset.conll.get(["word", "upos"]))
    ...
    if args.get("dict_only", False):
        ...
        global_step = 0
        max_steps = len(train_dataloader) * args["num_epoch"]
        max_dev_steps = len(dev_dataloader)
        dev_score_history = []
        ...
        for epoch in range(1, args["num_epoch"] + 1):
            ...
            dev_preds = trainer.postprocess(dev_dataset.conll.get(["word"]), dev_preds)

            # try ensembling with dict if necessary
            if args.get("ensemble_dict", False):
                print("[Ensembling dict with seq2seq model...]")
                dev_preds = trainer.ensemble(dev_dataset.conll.get(["word", "upos"]), dev_preds)
            dev_dataset.conll.write_conll_with_lemmas(dev_preds, system_pred_file)
            ...
            # lr schedule
            if (
                epoch > args["decay_epoch"]
                and dev_score <= dev_score_history[-1]
                and args["optim"] in ["sgd", "adagrad"]
            ):
...
def evaluate(args):
    ...
    if loaded_args["lemmatizer"] is None:
        ...
        lemmatizer = ExtendedLexicon(
            trainer.lexicon, importlib.import_module("lexenlem.lemmatizers." + args["lemmatizer"])
        )
    else:
    ...
    dict_preds = trainer.predict_dict(eval_dataset.conll.get(["word", "upos"]))
    ...
    if loaded_args.get("dict_only", False):
                            ...
                            if log_attns[k].shape[0] > attns[k].shape[0]:
                                attns[k] = np.vstack(
                                    (
                                        ...
                                        np.zeros(
                                            (
                                                log_attns[k].shape[0] - attns[k].shape[0],
                                                attns[k].shape[1],
                                                attns[k].shape[2],
                                            )
                                        ),
                                ...
                                log_attns[k] = np.vstack(
                                    (
                                        log_attns[k],
                                        np.zeros(
                                            (
                                                attns[k].shape[0] - log_attns[k].shape[0],
                                                log_attns[k].shape[1],
                                                log_attns[k].shape[2],
                                            )
                                        ),
        ...
        preds = trainer.postprocess(eval_dataset.conll.get(["word"]), preds)

        if loaded_args.get("ensemble_dict", False):
            print("[Ensembling dict with seq2seq lemmatizer...]")
            preds = trainer.ensemble(eval_dataset.conll.get(["word", "upos"]), preds)

    # write to file and score
    eval_dataset.conll.write_conll_with_lemmas(preds, system_pred_file)
